      ...
      <DashboardLayout>
        <div className="space-y-5 sm:space-y-6 md:space-y-8">
          <div className="min-w-0">
            <h1 className="font-display text-xl sm:text-2xl md:text-3xl font-bold truncate">Coaching</h1>
          ...
          {mySessions.length > 0 && (
            <div className="space-y-3 sm:space-y-4">
              <h2 className="font-display text-base sm:text-lg font-semibold">My Sessions</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                {mySessions.map((session) => (
                  <div key={session.id} className="dashboard-card p-4 sm:p-5 rounded-xl sm:rounded-2xl">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {filteredCoaches.map((coach) => (
                <div key={coach.id} className="dashboard-card p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-display text-base sm:text-lg font-semibold truncate">{coach.full_name}</h3>
                      {coach.bio && (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 mt-2">
                        <div className="text-xs sm:text-sm text-muted-foreground truncate">
                          From {formatCurrency(coach.hourly_rate || PRICING.coaching.private)}/hr
                        </div>
                        <Button onClick={() => handleSelectCoach(coach)} className="w-full sm:w-auto h-9 sm:h-10 text-xs sm:text-sm shrink-0">
                          Book Session
                        </Button>
                      </div>
                    </div>
            ...
            <div className="text-center py-10 sm:py-16">
              ...
              <h3 className="font-display text-lg sm:text-xl font-semibold mb-2">No coaches available</h3>
              <p className="text-muted-foreground text-sm sm:text-base">Check back soon for available coaches.</p>
            </div>
          )}
        </div>
        ...
        <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
          <DialogContent className="max-w-lg max-h-[92vh] sm:max-h-[90vh] overflow-hidden flex flex-col p-4 sm:p-6">
            <DialogHeader>
